...
submission-grade output across five formats. The
[Comparison article](comparison.html) maps the idioms across packages.
:::
...
`footnotes` fill sections 2 and 4, `headers()` and `cols()` build the
column band in section 3, `subgroup()` adds the banner, and `preset()`
...
- **[Architecture](architecture.html)** — the engine phases in depth.
```
