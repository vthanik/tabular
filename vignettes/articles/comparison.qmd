---
title: "Coming from gt, rtables, gtsummary, flextable, or huxtable"
description: >
  Honest positioning: where tabular fits among the R table packages, when
  to reach for each, and how the idioms translate.
---
...
Table packages split along a single axis: do they **compute** the summary,
or do they **render** a summary you bring?
...
four-section page layout, decimal alignment via font metrics, and a CDISC
ARS audit trail. It does no statistics by design; you pair it with
...
- **gtsummary** — you have patient-level data and want the package to
  *compute* the demographics/AE summary. It is the fastest path from ADaM
  to a summary, and it renders through `gt`. Use it upstream of
...
- **rtables** — you want a powerful layout engine that *also* computes,
  ...
  for clinical output. Reach for `tabular` instead when you already have a
  wide summary and want native font-metric decimal alignment, the
  four-section layout, and the ARS manifest without learning a layout DSL.
- **gt** — best-in-class HTML/print tables for reports, papers, and
  dashboards. Unbeatable for general-purpose display tables; not built for
  paginated multi-page submission documents or decimal-metric alignment.
...
| Label a column | `cols_label()` | `var_labels()` | `col_spec(label =)` in `cols()` |
...
| Style cells | `tab_style()` + `cells_*()` | `cell_fmt` / formats | `style()` + `cells_*()` |
| Titles / footnotes | `tab_header()` / `tab_footnote()` | `main_title()` / `fnotes` | `titles =` / `footnotes =` |
| Save to a format | `gtsave()` | `export_as_rtf()` etc. | `emit()` |

The shapes rhyme deliberately: a `cells_*()` location plus a `style()`
verb will feel familiar to a `gt` user, and `usage = "group"` plays the
role of `gt`'s row groups or `rtables`' row splits.
...
*pre-summarised clinical table* rendered to *paginated, submission-grade*
output across all five formats with a built-in QC and audit trail — and
there it does something none of the others do end to end.

## Where to next

- **[Get started](../articles/tabular.html)** — your first table.
- **[From a cards ARD](from-ard.html)** — the `gtsummary`/`cards`
  hand-off in practice.
- **[Clinical cookbook](clinical-cookbook.html)** — six complete tables.
```
