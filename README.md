...
The same `tab` renders to markdown for a quick look (the demographics table below is the actual `md` backend output), and to a self-contained HTML page, a paginated PDF, a `tabularray` LaTeX fragment, or native OOXML `.docx` — all from the one spec:
