...
Native emission of clinical tables, figures, and listings to RTF,
LaTeX, HTML, PDF, and DOCX from a single dplyr-style API. Designed
for CDISC ADaM workflows and FDA / EMA regulatory submissions.

`tabular` replaces the `tfrmt` + `r2rtf` + `flextable` + `gt`
stitched-together stack with one package per format -- decimal
alignment, group-aware pagination, page-field tokens, and inline
markup that survives string operations all handled natively.
...
Five pre-summarised wide-format tables ship with the package to
power examples, vignettes, and tests:
