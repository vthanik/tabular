...
`check_latex()` reports which LaTeX packages are present and prints the
exact `tlmgr_install()` line for anything missing; `check_fonts(spec)`
does the same for the fonts a spec asks for, per backend.
