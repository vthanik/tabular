...
> RTF, LaTeX, HTML, PDF, and DOCX from one verb pipeline. A zero-row table
...
PDF is the only backend that shells out. Install [`tinytex`](https://yihui.org/tinytex/)
once per machine and `tabular` compiles with `xelatex` thereafter:
...
local TeX cannot resolve them — so PDF also works on locked-down
servers (Domino, Posit Workbench) where `tlmgr install` is impossible.
...
for anything genuinely missing; `check_fonts(spec)` does the same for
the fonts a spec asks for, per backend.
