...
* `group_rows()` is the new structural verb: `group_rows(by, display, skip)` names only the structural grouping keys outer to inner (the section headers and any hidden break keys, not the visible label column, which stays an ordinary `cols()` column and is auto-indented). `display` is a single value applied to every key (`"header_row"`, `"column"`, or `"column_repeat"`); a hidden break-only key is expressed with `col_spec(visible = FALSE)` rather than a display mode. `skip` names the key(s) that get a blank spacer between groups (e.g. `skip = "param"`), deriving from `display` when `NULL`. Nesting is just listing the header keys (`by = c("param", "visit")`). It replaces a prior declaration wholesale, and its keys may not overlap `subgroup(by = )`.
