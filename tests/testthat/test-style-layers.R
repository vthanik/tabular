...
test_that("style(template, where = ...) errors — at is required for templates", {
  expect_error(
    style_template() |> style(where = TRUE, bold = TRUE),
...
test_that("at and where together is a user error", {
  expect_error(
    tabular(saf_demo) |>
      style(bold = TRUE, where = TRUE, at = cells_body()),
    class = "tabular_error_input"
  )
})

test_that("neither at nor where is a user error", {
  expect_error(
...
test_that("legacy where-path still appends a style_predicate", {
  spec <- tabular(saf_demo) |> style(where = TRUE, bold = TRUE)
  expect_length(spec@styles@predicates, 1L)
  expect_length(spec@styles@layers, 0L)
})
