...
.typst_prelude <- function(
  ...
  c(
    ...
    ")",
    sprintf(
      "#set text(font: %s, size: %spt)",
      .typst_font_array(.effective_font_family(preset)),
      .typst_num(size)
    ),
    # Line pitch parity with the LaTeX backend: leading is the gap above
    # the line box, so `baseline_ratio - 1` em matches `\fontsize{s}{s *
    # ratio}`.
    sprintf(
      "#set par(leading: %sem)",
      .typst_num(round(.tabular_baseline_ratio - 1, 4L))
    ),
